...
class WanderP2(pygame.sprite.Sprite):
    def __init__(self):
        ...
        pygame.mixer.init()
        caminho_musica = os.path.join(diretorio_wanderClass, 'assets', '5.ogg')
        caminho_musica_1 = os.path.join(diretorio_wanderClass, 'assets', '6.ogg')
        caminho_musica_2 = os.path.join(diretorio_wanderClass, 'assets', '7.ogg')
        caminho_musica_3 = os.path.join(diretorio_wanderClass, 'assets', '14.ogg')
        ...
        caminho_wander_1 =  os.path.join(diretorio_wander, '01_wander.wav')
        caminho_wander_2 =  os.path.join(diretorio_wander, '02_wander.wav')
        caminho_wander_3 =  os.path.join(diretorio_wander, '03_wander.wav')
        caminho_wander_4 =  os.path.join(diretorio_wander, '04_wander.wav')
        self.grito1 = pygame.mixer.Sound(caminho_wander_1)
        self.grito2 = pygame.mixer.Sound(caminho_wander_2)
        self.grito3 = pygame.mixer.Sound(caminho_wander_3)
        self.grito4 = pygame.mixer.Sound(caminho_wander_4)
        ...
        self.load_sprites('attack1', start_row=6, num_frames=8)
        self.load_sprites('attack2', start_row=7, num_frames=18)
        self.load_sprites('attack3', start_row=8, num_frames=26)
        self.load_sprites('super_attack', start_row=9, num_frames=29)
        self.load_sprites('block', start_row=10, num_frames=8)
        self.load_sprites('roll', start_row=5, num_frames=6)
        self.load_sprites('air_attack', start_row=4, num_frames=7)
        self.load_sprites('hit', start_row=11, num_frames=6)
        self.load_sprites('up',start_row=2,num_frames=3)
        self.load_sprites('down',start_row=3,num_frames=3)
        self.load_sprites('die',start_row=12,num_frames=20)
    ...
    def update(self, keys, plataformas, other,modo):
        ...
        if not self.is_blocking and keys[pygame.K_i] and (self.current_time - self.last_block_time >= 5000):
    ...
    def update_attack_rect(self):
        if self.current_action == 'attack1':
            if self.flip:
                if 1 <= self.index < 5:
                    self.attack_rect = pygame.Rect(self.rect.x + 225, self.rect.y + self.rect.height - 70, 70, 45)  # Retângulo de ataque 1
                else:
                    self.attack_rect = pygame.Rect(0, 0, 0, 0)  # Ataque inativo
            else:
                if 1 <= self.index < 7:
                    self.attack_rect = pygame.Rect(self.rect.x + 280, self.rect.y + self.rect.height - 70, 70, 45)  # Retângulo de ataque 1
                else:
                    self.attack_rect = pygame.Rect(0, 0, 0, 0)  # Ataque inativo
            ...
            if self.flip:
                if 0 <= self.index < 3:
                    ...
                    self.attack_rect = pygame.Rect(self.rect.x + 190, self.rect.y + self.rect.height - 95, 90, 80)
                else:
                    self.attack_rect = pygame.Rect(0, 0, 0, 0)  # Ataque inativo
            else:
                if 0 <= self.index < 3:
                    ...
                    self.attack_rect = pygame.Rect(self.rect.x + 300, self.rect.y + self.rect.height - 95, 90, 80)
                else:
                    self.attack_rect = pygame.Rect(0, 0, 0, 0)  # Ataque inativo
            ...
            if self.flip:
                if 0 <= self.index < 3:
                    ...
                    self.attack_rect = pygame.Rect(self.rect.x + 190, self.rect.y + self.rect.height - 95, 90, 80)
                elif 14<= self.index <= 15:
                    self.attack_rect = pygame.Rect(self.rect.x + 215, self.rect.y + self.rect.height - 40, 50, 50)
                elif 16<= self.index < 17:
                    self.attack_rect = pygame.Rect(self.rect.x + 190, self.rect.y + self.rect.height - 90, 50, 90)
                elif 17<= self.index < 18:
                    self.attack_rect = pygame.Rect(self.rect.x + 85, self.rect.y + self.rect.height - 90, 70, 90)
                elif 18<= self.index < 22:
                    self.attack_rect = pygame.Rect(self.rect.x + 85, self.rect.y + self.rect.height - 90, 70, 90)
                else:
                ...
                if 0 <= self.index < 3:
                    ...
                    self.attack_rect = pygame.Rect(self.rect.x + 290, self.rect.y + self.rect.height - 40, 50, 50)
                elif 16<= self.index < 17:
                    self.attack_rect = pygame.Rect(self.rect.x + 320, self.rect.y + self.rect.height - 90, 50, 90)
                elif 17<= self.index < 18:
                    self.attack_rect = pygame.Rect(self.rect.x + 420, self.rect.y + self.rect.height - 90, 70, 90)
                elif 18<= self.index < 22:
                    self.attack_rect = pygame.Rect(self.rect.x + 420, self.rect.y + self.rect.height - 90, 70, 90)
                else:
                    self.attack_rect = pygame.Rect(0, 0, 0, 0)  # Ataque inativo # Ataque inativo  
            ...
            if self.flip:
                if 2 <= self.index < 8:
                    self.attack_rect = pygame.Rect( self.rect.x + 170, self.rect.y + 160, 100, 60) # Air ataque
                else:
                    self.attack_rect = pygame.Rect(0, 0, 0, 0)  # Ataque inativo
            else:
                if 2 <= self.index < 8:
                    self.attack_rect = pygame.Rect(self.rect.x + 305 , self.rect.y + 160, 100, 60)  # Air ataque
                else:
                    self.attack_rect = pygame.Rect(0, 0, 0, 0)  # Ataque inativo
        elif self.current_action == 'super_attack':
            if self.flip:
                if 12 <= self.index <= 28:
                    self.attack_rect = pygame.Rect(self.rect.x + 240, self.rect.y + self.rect.height - 100, 100, 100)  # Super ataque
                else:
                    self.attack_rect = pygame.Rect(0, 0, 0, 0)  # Ataque inativo
            else:
                if 10 <= self.index <= 24:
